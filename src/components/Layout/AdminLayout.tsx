...
import {
  ...
  Settings2, Bell, LineChart, Shield, Database, LogOut, Menu, X, Sun, Moon, KeyRound, Map as MapIcon, GraduationCap, MessageSquare, SlidersHorizontal, Lock, Eye, EyeOff, CheckCircle
} from 'lucide-react';
  ...
  const handleChangePwd = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      if (code === 'auth/wrong-password' || code === 'auth/invalid-credential') {
        ...
        setPwdError('Session expired. Please sign out and sign in again, then retry.');
      } else {
        setPwdError(result.error?.message || 'Failed to change password. Try again.');
      }
    }
  };
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-gray-800">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs text-gray-400 hover:bg-gray-800 hover:text-white transition-all font-medium"
            >
            ...
            <div className="p-6">
              {pwdSuccess ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">Your admin password has been updated successfully.</p>
                  <button
                    onClick={() => setShowChangePwd(false)}
                    className="px-6 py-2.5 rounded-xl bg-green-500 hover:bg-green-600 text-white text-sm font-semibold transition-colors"
                  >
                    Done
                  </button>
                </div>
                    ...
                    <div key={key}>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder={label}
                          required
                          className="w-full pl-9 pr-10 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition-all"
                        />
